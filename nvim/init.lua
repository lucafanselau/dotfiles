...
-- Sane default indentation (guess-indent.nvim overrides per-file)
vim.o.tabstop = 2
vim.o.shiftwidth = 2
vim.o.softtabstop = 2
...
-- Use OSC 52 to yank to the local (macOS) clipboard over SSH.
-- This works through SSH + tmux → your terminal (Ghostty) picks up
-- the escape sequence and writes to the system clipboard.
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      -- Alternative leader groups: <leader>f (file) / <leader>p (project) / <leader>g (git)
      ...
      vim.keymap.set('n', '<leader>fb', builtin.buffers, { desc = '[F]ile [B]uffers' })

      vim.keymap.set('n', '<leader>pg', builtin.live_grep, { desc = '[P]roject [G]rep' })
      vim.keymap.set('n', '<leader>pw', builtin.grep_string, { desc = '[P]roject current [W]ord' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<leader>ck', vim.lsp.buf.hover, '[C]ode Hover Doc')
          ...
          -- Toggle diagnostics (all signs, underlines, virtual text)
          ...
          map('<leader>cl', vim.diagnostic.open_float, '[C]ode [L]ine diagnostics')
        end,
      })
